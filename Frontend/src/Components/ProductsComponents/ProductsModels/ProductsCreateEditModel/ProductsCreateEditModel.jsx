...
import ChipListField from "./ProductsCreateEditModelComponents/ChipListField";
      ...
      >
        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex flex-col gap-5">
          ...
          <VariantSection
            ...
            onParentProductIdChange={(v) => form.setField("parentProductId", v)}
          ...
          <ChipListField
            ...
            input={form.newCategoryInput}
            setInput={form.setNewCategoryInput}
            onAdd={() =>
              form.addChip(
                "categoryIds",
                form.newCategoryInput,
                form.setNewCategoryInput,
              )
            }
            onRemove={(item) => form.removeChip("categoryIds", item)}
            error={errors.categoryIds}
          />
          <ChipListField
            label="Suppliers"
            items={formData.supplierIds}
            input={form.newSupplierInput}
            setInput={form.setNewSupplierInput}
            onAdd={() =>
              form.addChip(
                "supplierIds",
                form.newSupplierInput,
                form.setNewSupplierInput,
              )
            }
            onRemove={(item) => form.removeChip("supplierIds", item)}
          />
          <ChipListField
            label="Tags"
            items={formData.tags}
            input={form.newTagInput}
            setInput={form.setNewTagInput}
            onAdd={() =>
              form.addChip("tags", form.newTagInput, form.setNewTagInput)
            }
            onRemove={(item) => form.removeChip("tags", item)}
          />
          ...
          <FlagsSection
            ...
            onChange={(key, checked) => form.setField(["flags", key], checked)}
          />
