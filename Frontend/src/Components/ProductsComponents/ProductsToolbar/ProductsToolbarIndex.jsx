...
const ProductsToolbarIndex = () => {
      ...
      const sortOptions = ["Recently Added", "Name (A-Z)", "Stock (Low to High)", "Stock (High to Low)"];
    return (
        <div>
             {/* Toolbar */}
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-2 sm:gap-3 items-center">
            ...
            <div className="relative shrink-0 order-4 sm:order-3 w-full sm:w-auto">
              ...
              >
                {sortOptions.map((opt) => (
                  <option key={opt} value={opt} className="bg-[#0B2B22] text-emerald-300">
                    {opt}
                  </option>
                ))}
              </select>
            ...
            {/* Create */}
            <button className="flex items-center justify-center gap-1.5 h-10 px-3 sm:px-4 rounded-md bg-[#1D9E75] text-white text-[12px] font-medium border border-[#0F6E56] hover:bg-[#0F6E56] transition-colors shrink-0 whitespace-nowrap order-3 sm:order-4">
