...
import { FiX, FiCheck, FiDollarSign, FiAlertTriangle } from "react-icons/fi";
...
const OrderActionModal = ({ order, onClose, onUpdated }) => {
  ...
  const [view, setView] = useState(
    isPending ? "confirm" : dueAmount > 0 ? "pay" : "confirm",
  );
  const [payAmount, setPayAmount] = useState(dueAmount || 0);
  ...
  const changeAmount = Math.max(enteredAmount - dueAmount, 0);
  const remainingDue = Math.max(dueAmount - enteredAmount, 0);
  ...
  const handleConfirmClick = async () => {
    ...
    try {
      ...
      onUpdated(order._id, { status: "complete", ...(res.data?.data || {}) });
      onClose();
    } catch (err) {
  ...
  const handlePaySubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      // Record payment up to dueAmount (excess is returned as change)
      const amountToRecord = Math.min(amount, dueAmount);
      const payRes = await payOrder(order._id, amountToRecord);
      const updatedPaid = payRes.data?.data?.payment?.paidAmount ?? paidSoFar + amountToRecord;
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-emerald-300/30">
          <h3 className="font-bold text-emerald-900 text-sm">
            {view === "pay" ? "Collect Payment" : "Confirm Order"}
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between text-xs">
            <span className="text-emerald-700/60 font-semibold">Paid so far</span>
            <span className="font-bold text-emerald-900">
              {currency}
          ...
          {view === "confirm" && (
            <p className="text-[12px] text-emerald-700/60">
              {dueAmount > 0
                ? "This order still has money owed. Confirming will ask you to collect payment first."
                : "This order is fully paid. Confirm to mark it complete."}
            </p>
          )}
          ...
          {view === "pay" && (
            <div className="flex flex-col gap-2.5">
              ...
              {/* Dynamic Return / Change Calculation */}
              {enteredAmount > 0 && (
        ...
        <div className="p-4 border-t border-emerald-300/30 flex gap-2">
          ...
          <button
            type="button"
            onClick={view === "pay" ? handlePaySubmit : handleConfirmClick}
          ...
          >
            {view === "pay" ? <FiDollarSign size={14} /> : <FiCheck size={14} />}
            {isSubmitting
              ? "Saving..."
              : view === "pay"
