...
async function getServerPublicKey() {
  ...
  if (!res.ok) throw new Error(`Could not fetch public key (HTTP ${res.status})`)
...
function validateSignup(f) {
  ...
  if (score < 3)
    return `Password too weak — needs: ${PASS_RULES.filter(r => !r.re.test(f.password||'')).map(r=>r.label).join(', ')}.`
...
const SIGNUP_FIELDS = [
  ...
  { id: 'dob',      label: 'Date of Birth',    type: 'date',     placeholder: '',                 required: false, half: true,
      ...
      <div className="auth-card">
        ...
        <form className="auth-form" onSubmit={handleSubmit} noValidate>
          <div className="fields-grid">
            {fields.map(f => (
              <div key={f.id}
                className={['field-group', !f.half && 'field-group--full'].filter(Boolean).join(' ')}
              >
                <label htmlFor={f.id} className="field-label">
                  {f.label}
                  {f.required
                    ? <span className="req" title="Required"> *</span>
                ...
                <div className="field-wrap">
                  <input
                    ...
                    required={f.required}
                    autoComplete={
                      f.id === 'password' || f.id === 'confirm' ? 'new-password'
                      : f.id === 'email' ? 'email' : 'off'
                    }
                  />
                  {f.id === 'password' && (
...
<span title="AES key wrapped with server RSA-OAEP 2048-bit public key">🔐 RSA-OAEP 2048</span>
          <span title="Payload encrypted with AES-GCM 256-bit">🔒 AES-GCM 256</span>
        </div>
      </div>
    </div>
  )
